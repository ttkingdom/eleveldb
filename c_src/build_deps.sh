...
LEVELDB_VSN="macos-m1-docker-build"
...
case "$1" in
    ...
    *)
        ...
        if [ ! -d snappy-$SNAPPY_VSN ]; then
            ...
            (cd snappy-$SNAPPY_VSN && ./configure --build=aarch64-unknown-linux-gnu --disable-shared --prefix=$BASEDIR/system --libdir=$BASEDIR/system/lib --with-pic)
        fi
        ...
        if [ ! -d leveldb ]; then
            git clone https://github.com/ttkingdom/leveldb
            (cd leveldb && git checkout $LEVELDB_VSN)
            (cd leveldb && git submodule update --init)
        fi
